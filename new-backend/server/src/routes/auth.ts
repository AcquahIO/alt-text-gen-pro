...
async function issueLoginCode(userId: string, state: string, redirectUri: string) {
...
async function consumeLoginCode(code: string) {
  ...
  if (record.expiresAt.getTime() < Date.now()) throw new Error('Code expired');
...
router.get('/auth/google', async (req, res) => {
  ...
  const encodedState = Buffer.from(
    JSON.stringify({ redirect_uri: redirectUri, client_state: state, code_verifier: codeVerifier }),
    'utf8'
  ).toString('base64url');
...
router.get('/auth/google/callback', async (req, res) => {
  ...
  let statePayload: { redirect_uri: string; client_state: string; code_verifier: string };
  try {
  ...
  const email = (payload as JWTPayload & { email?: string; email_verified?: boolean }).email?.toLowerCase();
  ...
  const user = await prisma.user.upsert({
    where: { email },
    update: { googleSub: payload.sub as string, emailVerified: true },
    create: { email, emailVerified: true, googleSub: payload.sub as string },
  });
...
router.post('/auth/login/email', limiter, async (req, res, next) => {
  try {
    ...
    if (!user || !user.passwordHash) return res.status(400).send('Invalid credentials');
